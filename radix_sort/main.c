...
int main(void)
{
    ...
    for (i = 0; i< NUM_USERS; ++i) {
        ...
        users[i].sort_key = age << 16 | id; /*나이를 먼저 오름차순, 같다면 id를 오름차순*/
    }
    ...
    for (i = 0; i < NUM_USERS; ++i) {
        unsigned char inv_age = 255 - users[i].user.age; /*나이를 내림차순하기 위해 complement 연산*/
    ...
    qsort(users, NUM_USERS, sizeof(radix_userdata_t), compare_uint);
    for (i = 0; i < NUM_USERS; ++i) {
        printf("age: %3d, sex: %d, id: %d\n", users[i].user.age, users[i].user.sex, users[i].user.id);
    }
